...
const form = document.getElementById('transactionForm'); // Disesuaikan dengan ID HTML
...
// Format Angka Otomatis ke Rupiah untuk Dashboard Utama
...
function updateDashboard() {
    const income = transactions.filter(tx => tx.type === 'pemasukan' || tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0);
    const expense = transactions.filter(tx => tx.type === 'pengeluaran' || tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0);
...
function renderTransactions(searchQuery = '') {
    ...
    const incomeTransactions = filtered.filter(tx => tx.type === 'pemasukan' || tx.type === 'income');
    const expenseTransactions = filtered.filter(tx => tx.type === 'pengeluaran' || tx.type === 'expense');
    ...
    function renderGroupedData(groupedData, targetContainer) {
        ...
        sortedDates.forEach(date => {
            ...
            groupedData[date].forEach(tx => {
                ...
                card.className = `transaction-card bg-slate-50/80 p-4 rounded-xl border border-slate-100 flex flex-col gap-3 transition-all hover:bg-slate-100/80 shadow-sm`;
                
                // 1. DATA-TESTID UNTUK KARTU UTAMA (Wajib memakai camelCase tanpa strip)
                card.setAttribute('data-testid', 'transactionItem');

                const isIncome = tx.type === 'pemasukan' || tx.type === 'income';
                const badgeColor = isIncome ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-[#FFEBEE] text-[#C62828]';
                const typeText = isIncome ? 'Pemasukan' : 'Pengeluaran';

                // 2. STRUKTUR TEMPLAT SESUAI KETENTUAN REVIEWER (Menjaga kelas style bawaan kamu tetap indah)
                card.innerHTML = `
                    <div class="flex items-start justify-between w-full">
                        <div class="space-y-1">
                            <span class="px-2 py-0.5 rounded-md text-[9px] font-bold ${badgeColor} inline-block mb-1">
                                ${tx.category}
                            </span>
                            <h3 data-testid="transactionItemTitle" class="font-semibold text-sm text-slate-700 block">${tx.title}</h3>
                            <p data-testid="transactionItemDate" class="text-[11px] text-slate-400">Tanggal: ${tx.date}</p>
                            <p data-testid="transactionItemType" class="text-[11px] font-medium ${isIncome ? 'text-green-600' : 'text-red-500'}">Tipe: ${typeText}</p>
                        </div>
                        <p data-testid="transactionItemAmount" class="font-bold text-sm ${isIncome ? 'text-green-600' : 'text-red-500'}">
                            Nominal: Rp${tx.amount}
                        </p>
                    </div>
                    
                    <div class="flex items-center justify-end text-[11px] text-slate-400 border-t border-slate-200/40 pt-2 mt-1">
                        <div class="flex items-center gap-2">
                            <button data-testid="transactionItemEditTypeButton" onclick="toggleType(${tx.id})" class="text-indigo-500 hover:text-indigo-700 font-semibold bg-indigo-50 px-2 py-1 rounded transition-colors">
                                <i class="fa-solid fa-repeat"></i> Ubah Tipe
                            </button>
                            <span>|</span>
                            <button data-testid="transactionItemDeleteButton" onclick="deleteTransaction(${tx.id})" class="text-rose-500 hover:text-rose-700 font-semibold bg-rose-50 px-2 py-1 rounded transition-colors">
                                🗑️ Hapus
                            </button>
                        </div>
                    </div>
...
form.addEventListener('submit', (e) => {
    ...
    // Format tanggal standard ISO (YYYY-MM-DD) agar mesin test otomatis mudah membacanya
    ...
    const newTransaction = {
        ...
        type: selectedType, // Menyimpan string 'pemasukan' atau 'pengeluaran'
    ...
    saveData();
    form.reset();
    document.querySelector('input[value="pemasukan"]').checked = true;
...
window.toggleType = function(id) {
    transactions = transactions.map(tx => {
        if (tx.id === id) {
            const newType = (tx.type === 'pemasukan' || tx.type === 'income') ? 'pengeluaran' : 'pemasukan';
            return { ...tx, type: newType };
        }
        return tx;
    });
    saveData();
